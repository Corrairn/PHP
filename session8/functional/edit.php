...
function loadUser($id) {
	...
	$stmt = $pdo->prepare("SELECT * FROM users WHERE `ID` = :name");
	$stmt->bindParam(':name', $_GET['id']);
	$stmt->execute();
...
function updateUser($id, $username, $password) {
	...
	$query =
		...
		SET
			...
			`password` = :password
		WHERE
			`ID` = :id"; // Конструиране на заявката
	
	$stmt = $pdo->prepare($query);
	$stmt->bindParam(':username', $username); // Добавяне на параметрите username
	$stmt->bindParam(':password', $password); // password
	$stmt->bindParam(':id', $id); // и id.


	if($stmt->execute()) { // Изпълняване, и при успех показване на суобщение.
		echo "The user with ID {$id} was updated.";
	}
}
